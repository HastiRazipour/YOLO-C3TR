...
class C3TR(nn.Module):
    """C3 with Transformer layers (برای ترکیب YOLOv5 با Transformer)"""

    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super(C3TR, self).__init__()
        c_ = int(c2 * e)  # کانال‌های میانی
        self.cv1 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.cv2 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.cv3 = nn.Conv2d(2 * c_, c2, 1, 1, bias=False)
        self.m = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=c_, nhead=4), num_layers=n
        )  # لایه ترنسفورمر

    def forward(self, x):
        x1 = self.cv1(x)
        x2 = self.cv2(x)
        x2 = self.m(x2.flatten(2).permute(2, 0, 1))  # تغییر شکل داده‌ها برای ترنسفورمر
        x2 = x2.permute(1, 2, 0).view_as(x1)  # بازگرداندن به شکل اولیه
